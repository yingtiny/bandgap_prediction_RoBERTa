...
def analyze_crystal_embeddings(npy_file, data_file, save_dir='0522_llama_pretrained_embedding_analysiss'):
    ...
    if len(texts) != embeddings.shape[0]:
        raise ValueError(f"Embeddings is not matching with amount of text, Embeddings: {embeddings.shape[0]}, Texts: {len(texts)}")
...
if __name__ == "__main__":
    npy_file = './0522_llama_pretrained_embedding_analysis/encoder_embeddings.npy'
    data_file = './data/df_aflow_4.pkl'
    crystal_counts = analyze_crystal_embeddings(
        npy_file=npy_file,
        data_file=data_file,
        save_dir='0522_llama_pretrained_embedding_analysis'
    )