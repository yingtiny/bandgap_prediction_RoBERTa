...
def analyze_crystal_embeddings(npy_file, data_file, save_dir='embedding_analysis_pre_1112'):
    ...
    if len(texts) != embeddings.shape[0]:
        raise ValueError(f"not matching, Embeddings: {embeddings.shape[0]}, Texts: {len(texts)}")
    ...
    plt.legend(title='Crystal System', loc='lower right',fontsize=19,title_fontsize=19)#, bbox_to_anchor=(1.05, 1)
    plt.xticks([])
    plt.yticks([])
    plt.xlabel('')  
    plt.ylabel('') 
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/crystal_system_embeddings.png', 
...
if __name__ == "__main__":
    npy_file = './embedding_analysis_pre/pretrained_cls_embeddings.npy'
    data_file = './data/df_aflow_4.pkl'
    
    crystal_counts = analyze_crystal_embeddings(
        npy_file=npy_file,
        data_file=data_file,
        save_dir='embedding_analysis_pre_1112'
    )