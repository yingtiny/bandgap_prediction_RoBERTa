...
class Network(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, output_attentions=None):
        ...
        prediction = self.head(base_outputs.pooler_output)
        if output_attentions:
            return prediction, base_outputs.attentions
        return prediction
        


def create_model(config):
    bert_config = RobertaConfig.from_pretrained('roberta-base',output_attentions=True)
