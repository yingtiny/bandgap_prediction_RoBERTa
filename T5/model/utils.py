...
def train(model, dataloader, criterion, mae_loss, optimizer, device):
    ...
    for i, batch in enumerate(bar):
        ...
        loss_mae = mae_loss(outputs, targets)
...
def train_model(config, model, train_loader, valid_loader, test_loader, criterion, mae_loss, optimizer, scheduler):
    ...
    if not config.debug:
        wandb.init(project='T5_0407', name=config.run_name)
    ...
    if not config.debug:
        ...
        save_results(config.run_name, best_r2, loss_over_epochs_train[-1], 
                    loss_over_epochs_valid[-1], loss_over_epochs_test[-1], 
                    current_lr, config.batch_size, config.epochs, 
                    best_model_time, total_training_time)
